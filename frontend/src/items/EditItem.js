...
export default function EditItem() {
  ...
  const onSubmit = async (e) => {
    ...
    //await axios.put(`http://ccelectricalservices.eu-west-1.elasticbeanstalk.com/items/${id}`, item);
    // Local Development URL
    await axios.put(`http://localhost:8082/items/${id}`, item);
    redirect("/");
    //navigate("/");
  };
  ...
  const loadItem = async () => {
    // Production URL (AWS Elastic Beanstalk)
    //const result = await axios.get(`http://ccelectricalservices.eu-west-1.elasticbeanstalk.com/items/${id}`);
    // Local Development URL
    const result = await axios.get(`http://localhost:8082/items/${id}`);
    setItem(result.data);
  };
